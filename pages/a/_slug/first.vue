...
export default {
  ...
  async fetch({ store, params, redirect, error }) {
    ...
    const viewingEpisode = store.getters.viewingEpisode;
    ...
    const firstEpisode = episodes.length > 0 ? episodes[0].number : 1;
    const episode = viewingEpisode && viewingEpisode.anime_id === viewingAnime.id
      ? viewingEpisode.number
      : firstEpisode;

    return redirect(`/a/${params.slug}/${episode}`);
  },
};
</script>
