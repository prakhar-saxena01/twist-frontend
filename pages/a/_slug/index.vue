...
export default {
  ...
  async fetch({ store, params, redirect, error }) {
    ...
    const lastWatched = store.getters.getLastWatchedLibraryEpisode(viewingAnime);
    const nextToWatch = store.getters.getNextEpisodeToWatch(viewingAnime);
    const alwaysResumeEpisode = store.getters.alwaysResumeEpisode;

    if (alwaysResumeEpisode && lastWatched && !lastWatched.completed) {
      const lastWatchedEpisode = store.getters.libraryToEpisode({
        library: lastWatched,
        anime: viewingAnime,
      });
      return redirect(`/a/${params.slug}/${lastWatchedEpisode.number}`);
    } else if (alwaysResumeEpisode && nextToWatch) {
      return redirect(`/a/${params.slug}/${nextToWatch.number}`);
    }

    return redirect(`/a/${params.slug}/first`);
  },
};
</script>
